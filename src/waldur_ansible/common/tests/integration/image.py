...
class DockerImage:
    ...
    def import_base_image(self, base_image_file_name):
        docker.APIClient().import_image(
            src=base_image_file_name, repository=self.base_image_file_name
        )

    def download_base_image(self):
        base_image_file_path = '%s/%s/%s.%s' % (
            os.path.dirname(os.path.abspath(__file__)),
            self.image_dir_name,
            self.base_image_file_name,
            self.base_image_file_extension,
        )
    ...
    def build_image_from_dockerfile(self):
        _, result_stream = docker.from_env().images.build(
            rm=True,
            path='%s/%s'
            % (os.path.dirname(os.path.abspath(__file__)), self.image_dir_name),
            tag=self.image_name,
        )
