...
class SmaxServiceBackend(SupportBackend):
    ...
    def update_waldur_issue_from_smax(self, issue):
        ...
        try:
            ...
            for backend_comment in issue_comments:
                ...
                if existing_users.count() > 1:
                    logger.error(
                        f"Multiple SupportUser objects found for backend_id={backend_user.id}. "
                        f"Count: {existing_users.count()}, User IDs: {list(existing_users.values_list('id', flat=True))}"
                    )
                    # Use the first one for now
                    support_user = existing_users.first()
                    created = False
                else:
        ...
        except models.SupportUser.MultipleObjectsReturned as e:
            logger.error(
                f"Multiple SupportUser objects returned when updating issue {issue.id}: {str(e)}"
            )
            # Additional diagnostics
            if "backend_user" in locals():
                duplicates = models.SupportUser.objects.filter(
                    backend_id=backend_user.id,
                    backend_name=self.backend_name,
                )
                logger.error(
                    f"Duplicate SupportUser objects for backend_id={backend_user.id}: "
                    f"IDs={list(duplicates.values_list('id', flat=True))}"
                )
