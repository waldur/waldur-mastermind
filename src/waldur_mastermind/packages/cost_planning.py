...
from waldur_mastermind.packages.models import PackageComponent, PackageTemplate
...
class OpenStackOptimizer(optimizers.Optimizer):
    ...
    def optimize(self, deployment_plan, service):
        ...
        for template in templates:
            ...
            is_suitable = (
                components[PackageComponent.Types.RAM] >= requirements['ram']
                and components[PackageComponent.Types.STORAGE]
                >= requirements['storage']
                and components[PackageComponent.Types.CORES] >= requirements['cores']
            )
            if is_suitable:
        ...
        if not suitable_templates:
            raise optimizers.OptimizationError(
                'There is no package template that can hold the deployment plan.'
            )
        cheapest_template = min(suitable_templates, key=lambda t: t.price)
        return OptimizedOpenStack(
            service, cheapest_template.price, '', cheapest_template
        )


register.Register.register_optimizer(
    openstack_apps.OpenStackConfig.service_name, OpenStackOptimizer
)


class OptimizedOpenStackSerializer(serializers.OptimizedServiceSerializer):
    service = rf_serializers.HyperlinkedRelatedField(
        view_name='openstack-detail', lookup_field='uuid', read_only=True,
    )
...
register.Register.register_serializer(
    openstack_apps.OpenStackConfig.service_name, OptimizedOpenStackSerializer
)
