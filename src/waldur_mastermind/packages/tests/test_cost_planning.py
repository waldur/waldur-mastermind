...
class PackageDeploymentPlanTest(test.APITransactionTestCase):
    def setUp(self):
        ...
        self.plan = cost_planning_factories.DeploymentPlanFactory(
            project=self.fixture.project
        )
        self.url = cost_planning_factories.DeploymentPlanFactory.get_url(
            self.plan, action='evaluate'
        )
    ...
    def test_user_can_evaluate_deployment_plan(self):
        ...
        settings_url = structure_factories.ServiceSettingsFactory.get_url(
    ...
    def test_filter_package_template_if_items_quantity_more_one_and_template_component_meet_plan_requirements(
        self,
    ):
        quantity = 2

        for response in self._get_responses_for_different_template_components(
            masks=[(1, 1, 1),], quantity=quantity
        ):
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            plan = response.data[0]
            template = plan['package_template']
            ...
            self.assertEqual(
                components['storage'], self.preset_param['storage'] * quantity
            )

    def test_filter_package_template_if_items_quantity_more_one_and_template_component_dont_meet_plan_requirements(
        self,
    ):
        quantity = 2

        for response in self._get_responses_for_different_template_components(
            masks=[(0, 0, 0), (1, 0, 0), (1, 1, 0)], quantity=quantity
        ):
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            plan = response.data[0]
    ...
    def _get_responses_for_different_template_components(self, masks, quantity):
        ...
        for k in masks:
            cmpt_param = {
                'ram': self.preset_param['ram'] * k[0] * quantity
                or self.preset_param['ram'],
                'cores': self.preset_param['cores'] * k[1] * quantity
                or self.preset_param['cores'],
                'storage': self.preset_param['storage'] * k[2] * quantity
                or self.preset_param['storage'],
            }
