...
class OpenStackPackageCreateTest(test.APITransactionTestCase):
    ...
    def test_if_only_staff_manages_services_he_can_create_openstack_package(self, user):
        response = common_utils.create_request(self.view, getattr(self.fixture, user), self.get_valid_payload())
    ...
    def test_manager_can_create_openstack_package_with_permission_from_settings(self):
        response = common_utils.create_request(self.view, self.fixture.manager, self.get_valid_payload())
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_admin_can_create_openstack_package_with_permission_from_settings(self):
        response = common_utils.create_request(self.view, self.fixture.admin, self.get_valid_payload())

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_tenant_quotas_are_defined_by_template(self):
        response = common_utils.create_request(self.view, self.fixture.owner, self.get_valid_payload())
...
class OpenStackPackageChangeTest(test.APITransactionTestCase):
    ...
    def test_package_is_replaced_on_extend(self):
        response = common_utils.create_request(self.view, self.fixture.staff, self.get_valid_payload())
    ...
    def test_logger_called_when_package_change_scheduled(self, logger_mock):
        common_utils.create_request(self.view, self.fixture.staff, self.get_valid_payload())
