...
def log_resource_created(sender, instance, **kwargs):
    ...
    if not client:
        return
    title = 'Resource {resource_name} has been created.'.format(
        resource_name=instance.full_name
    )
    point = format_event({'title': title, 'type': 'resource_created',})
...
def log_resource_deleted(sender, instance, **kwargs):
    ...
    if not client:
        return
    title = 'Resource {resource_name} has been deleted.'.format(
        resource_name=instance.full_name
    )
    point = format_event({'title': title, 'type': 'resource_deleted',})
    utils.write_points(client, [point])
