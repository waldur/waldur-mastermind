...
from waldur_mastermind.chat.injection_detection.base import BaseDetector
from waldur_mastermind.chat.injection_detection.regex_detector import (
    SEVERITY_ACTION_MAP,
    RegexDetector,
)
...
class DetectionResultTest(unittest.TestCase):
    """Tests for the DetectionResult dataclass defaults and structure."""
    ...
    def test_mutable_defaults_are_independent(self):
        """Each instance must get its own list/dict, not a shared mutable."""
        r1 = DetectionResult(
            is_injection=False,
            score=0.0,
            severity=SeverityLevel.NONE,
...
class SeverityActionMapTest(unittest.TestCase):
    ...
    def test_none_maps_to_allow(self):
        self.assertEqual(SEVERITY_ACTION_MAP[SeverityLevel.NONE], DetectionAction.ALLOW)

    def test_low_maps_to_flag(self):
        self.assertEqual(SEVERITY_ACTION_MAP[SeverityLevel.LOW], DetectionAction.FLAG)

    def test_medium_maps_to_flag(self):
        self.assertEqual(
            SEVERITY_ACTION_MAP[SeverityLevel.MEDIUM], DetectionAction.FLAG
        )

    def test_high_maps_to_block(self):
        self.assertEqual(SEVERITY_ACTION_MAP[SeverityLevel.HIGH], DetectionAction.BLOCK)

    def test_critical_maps_to_block(self):
        self.assertEqual(
            SEVERITY_ACTION_MAP[SeverityLevel.CRITICAL], DetectionAction.BLOCK
        )

    def test_all_severity_levels_are_mapped(self):
        for level in SeverityLevel:
            self.assertIn(level, SEVERITY_ACTION_MAP)
...
class RegexDetectorMetadataTest(unittest.TestCase):
    ...
    def test_name_returns_regex(self):
        ...
        self.assertEqual(detector.name, "regex")
...
class RegexDetectorAllowlistTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._config_patcher = mock.patch(
            "waldur_mastermind.chat.injection_detection.regex_detector.config"
        )
...
class RegexDetectorScoreThresholdTest(unittest.TestCase):
    ...
    def test_is_injection_boundary_at_0_30(self):
        """is_injection switches from False to True at score 0.30."""
        result_below = self._detect_with_weight(0.29)
        result_at = self._detect_with_weight(0.30)
        self.assertFalse(result_below.is_injection)
        self.assertTrue(result_at.is_injection)


class PromptInjectionServiceScopeTest(unittest.TestCase):
    """Tests for PromptInjectionService scope tagging in result details."""

    def setUp(self):
        self.detector = RegexDetector()
        self.service = PromptInjectionService(detectors=[self.detector])

    def test_check_user_input_sets_scope(self):
        result = self.service.check_user_input("Hello world")
        self.assertEqual(result.details.get("scope"), "user_input")

    def test_check_tool_arguments_sets_scope(self):
        result = self.service.check_tool_arguments("show_resources", {"user_id": "123"})
        self.assertEqual(result.details.get("scope"), "tool_arguments")
...
class PromptInjectionServiceMultiDetectorTest(unittest.TestCase):
    ...
    def test_highest_score_wins(self):
        ...
        service = PromptInjectionService(detectors=[low_detector, high_detector])
    ...
    def test_patterns_merged_across_detectors(self):
        ...
        service = PromptInjectionService(detectors=[det_a, det_b])
    ...
    def test_no_detectors_returns_allow(self):
        ...
        service = PromptInjectionService(detectors=[])
...
class PromptInjectionServiceToolArgumentsTest(unittest.TestCase):
    ...
    def test_tool_name_not_included_in_detection_text(self):
        ...
        service = PromptInjectionService(detectors=[detector])
    ...
    def test_empty_arguments_return_allow(self):
        ...
        service = PromptInjectionService(detectors=[RegexDetector()])
...
class PromptInjectionServiceMockDetectorTest(unittest.TestCase):
    ...
    def test_service_calls_all_detectors(self):
        ...
        mock_det1.detect.return_value = DetectionResult(
            is_injection=False,
            score=0.0,
            severity=SeverityLevel.NONE,
            action=DetectionAction.ALLOW,
        ...
        mock_det2.detect.return_value = DetectionResult(
            is_injection=False,
            score=0.0,
            severity=SeverityLevel.NONE,
            action=DetectionAction.ALLOW,
        ...
        service = PromptInjectionService(detectors=[mock_det1, mock_det2])
    ...
    def test_service_selects_highest_scoring_result(self):
        ...
        service = PromptInjectionService(detectors=[mock_det_low, mock_det_high])
