...
class GetToolsPromptTest(TestCase):
    ...
    def test_tool_without_parameters_shows_no_parameters(self):
        # list_projects has no parameters — verify via get_openai_tools()
        openai_tools = tool_registry.get_openai_tools()
        list_projects_tool = next(
            t for t in openai_tools if t["function"]["name"] == "list_projects"
        )
        params = list_projects_tool["function"]["parameters"]
...
class UICapabilitiesTest(TestCase):
    ...
    def test_tables_are_tool_only(self):
        self.assertIn("tables", UI_CAPABILITIES.lower())
        self.assertIn("only generated by tools", UI_CAPABILITIES.lower())
