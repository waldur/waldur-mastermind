...
def get_remote_offerings_for_project(project):
    offering_ids = (
        marketplace_models.Resource.objects.filter(project=project)
...
def create_project_permission(client, remote_project_uuid, remote_user_uuid, role):
...
def remove_project_permission(client, remote_project_uuid, remote_user_uuid, role):
    ...
    if remote_permissions:
        client.remove_project_permission(str(remote_permissions[0]['pk']))
        return True
    return False


def sync_project_permission(grant, project, role, user):
    for offering in get_remote_offerings_for_project(project):
        ...
        try:
            remote_project_uuid, _ = get_or_create_remote_project(
...
def collect_local_user_roles(project):
    ...
    for permission in structure_models.ProjectPermission.objects.filter(
        ...
        permissions[permission.user.username].add(permission.role)
    for permission in structure_models.CustomerPermission.objects.filter(
        ...
        permissions[permission.user.username].add(structure_models.ProjectRole.MANAGER)
    return permissions
