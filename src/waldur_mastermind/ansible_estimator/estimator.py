...
def get_job(request):
    ...
    """
    serializer = serializers.JobEstimateSerializer(
        data=request.data, context={'request': request}
    )
    serializer.is_valid(raise_exception=True)

    return InMemoryJob(name='VALID_JOB_NAME', **serializer.validated_data)
...
def get_prices(service_settings):
    ...
    if not tenant:
        raise exceptions.ValidationError(
            _('Provider is not connected to the OpenStack tenant.')
        )

    package_uuid = tenant.extra_configuration.get('package_uuid')
    if not package_uuid:
        raise exceptions.ValidationError(
            _('OpenStack tenant is not connected to the VPC package.')
        )
    ...
    try:
        ...
        raise exceptions.ValidationError(
            _('VPC package is not connected to the package template.')
        )
    ...
    try:
        return {
            'cpu': components.get(
                type=package_models.PackageComponent.Types.CORES
            ).price,
            'ram': components.get(type=package_models.PackageComponent.Types.RAM).price,
            'disk': components.get(
                type=package_models.PackageComponent.Types.STORAGE
            ).price,
        }
    except package_models.PackageComponent.DoesNotExist:
        raise exceptions.ValidationError(
            _('Package template does not have required components.')
        )
...
def evaluate_job(job):
    ...
    try:
        ...
        logger.debug(
            'Unable to process Ansible playbook. '
...
def get_resources(request, job):
    ...
    for item in items:
        ...
        serializer = openstack_serializers.InstanceSerializer(
        ...
        if not serializer.is_valid():
            ...
            quota_errors = list(
                itertools.chain(
                    ...
                    settings.validate_quota_change(
                        {settings.Quotas.ram: resource.flavor.ram}
                    ),
                    settings.validate_quota_change(
                        {settings.Quotas.vcpu: resource.flavor.cores}
                    ),
                    link.validate_quota_change({link.Quotas.ram: resource.flavor.ram}),
                    link.validate_quota_change(
                        {link.Quotas.vcpu: resource.flavor.cores}
                    ),
                )
            )
            if quota_errors:
                errors.append(
                    _('One or more quotas were exceeded: %s') % ';'.join(quota_errors)
                )
            else:
...
def get_requirements(resources):
    ...
    for resource in resources:
        ...
        requirements['disk'] += resource.system_volume_size + getattr(
            resource, 'data_volume_size', 0
        )
    return requirements
...
def get_report(request):
    ...
    try:
        ...
        logger.debug(
            'Unable to get prices for service settings %s. Error is %s.',
            service_settings,
            e,
        )
    ...
    return {'prices': prices, 'requirements': requirements, 'cost': cost}
