...
from waldur_openstack.openstack_tenant import apps as tenant_apps
...
class EstimationFixture(structure_fixtures.ProjectFixture):
    ...
    def template(self):
        template = package_models.PackageTemplate.objects.create(
            name='Premium package template', service_settings=self.shared_settings
        )
        for (type, price) in self.prices.items():
            package_models.PackageComponent.objects.create(
                type=type, price=price, template=template
            )
        return template
    ...
    def shared_service(self):
        return openstack_models.OpenStackService.objects.get(
            customer=self.customer, settings=self.shared_settings,
        )

    @cached_property
    def shared_link(self):
        return openstack_models.OpenStackServiceProjectLink.objects.get(
            service=self.shared_service, project=self.project,
        )
    ...
    def tenant(self):
        return openstack_models.Tenant.objects.create(
            ...
            extra_configuration={'package_uuid': self.template.uuid.hex},
        )
    ...
    def private_service(self):
        return tenant_models.OpenStackTenantService.objects.create(
            settings=self.private_settings, customer=self.customer,
        )

    @cached_property
    def private_link(self):
        return tenant_models.OpenStackTenantServiceProjectLink.objects.create(
            service=self.private_service, project=self.project,
        )

    @cached_property
    def image(self):
        return tenant_factories.ImageFactory(
            settings=self.private_settings, min_disk=10240, min_ram=1024
        )
    ...
    def subnet(self):
        return tenant_models.SubNet.objects.create(
            settings=self.private_settings, network=self.network,
        )
