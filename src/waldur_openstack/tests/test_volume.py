...
class VolumeExtendTestCase(test.APITransactionTestCase):
    ...
    def test_user_can_not_extend_volume_if_resulting_quota_usage_is_greater_than_limit(
        ...
        tenant = self.volume.tenant
        tenant.set_quota_usage("storage", self.volume.size)
        tenant.set_quota_limit("storage", self.volume.size + 512)
    ...
    def test_when_volume_is_extended_volume_type_quota_is_updated(self):
        ...
        tenant.set_quota_usage(key, self.volume.size / 1024)
...
class VolumeSnapshotTestCase(test.APITransactionTestCase):
    ...
    def test_user_can_create_volume_snapshot_if_storage_quotas_are_full(self):
        quota = volume_type_name_to_quota_name(self.volume.type.name)
        self.volume.tenant.set_quota_limit(quota, 0)
        self.volume.tenant.set_quota_limit("storage", 0)
        response = self.create_snapshot()
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_when_snapshot_is_created_volume_storage_quota_is_not_updated(self):
        key = "storage"
        old_usage = self.fixture.tenant.get_quota_usage(key)
...
class BaseVolumeCreateTest(test.APITransactionTestCase):
    ...
    def create_volume(self, **extra):
        payload = {
            "name": "Test volume",
            "service_settings": ServiceSettingsFactory.get_url(self.openstack_settings),
...
class VolumeRetypeTestCase(test.APITransactionTestCase):
    ...
    def test_when_volume_is_retyped_volume_type_quota_is_updated(self):
        ...
        tenant.add_quota_usage(old_type_key, self.volume.size / 1024)

        # Act
        self.retype_volume(self.admin, self.new_type)

        # Assert
        self.assertEqual(0, tenant.get_quota_usage(old_type_key))
        self.assertEqual(self.volume.size / 1024, tenant.get_quota_usage(new_type_key))

    @unittest.skip("Not stable in GitLab CI")
    def test_when_volume_is_retyped_volume_type_quota_for_tenant_is_updated(
        self,
    ):
        # Arrange
        tenant = self.volume.tenant
