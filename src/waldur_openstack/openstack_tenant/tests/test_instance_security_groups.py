...
def _instance_data(user, instance=None):
    ...
    tenant: Tenant = instance.service_settings.scope
    openstack_factories.FloatingIPFactory(tenant=tenant, runtime_state="DOWN")
    image = openstack_factories.ImageFactory(settings=tenant.service_settings)
    flavor = openstack_factories.FlavorFactory(settings=tenant.service_settings)
    ssh_public_key = structure_factories.SshPublicKeyFactory(user=user)
    subnet = openstack_factories.SubNetFactory(tenant=tenant)
    return {
        "name": "test-host",
