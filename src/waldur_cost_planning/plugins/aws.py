...
from waldur_aws import apps as aws_apps
from waldur_aws import cost_tracking as aws_cost_tracking
from waldur_aws import models as aws_models
from waldur_aws import serializers as aws_serializers
...
class AWSOptimizer(optimizers.Optimizer):
    ...
    def _get_size_prices(self, sizes, service):
        ...
        return {
            size: size_prices.get(size.backend_id, size.price) * self.HOURS_IN_DAY
            for size in sizes
        }
        ...
        for item in deployment_plan.items.all():
            ...
            if not sizes:
                ...
                raise optimizers.OptimizationError(
                    'It is impossible to create an instance for preset %s. It is too big.'
                    % preset_as_str
                )
...
class OptimizedAWSSerializer(serializers.OptimizedServiceSerializer):
    service = rf_serializers.HyperlinkedRelatedField(
        view_name='aws-detail', lookup_field='uuid', read_only=True,
    )
    optimized_presets = OptimizedPresetSerializer(many=True)


register.Register.register_serializer(
    aws_apps.AWSConfig.service_name, OptimizedAWSSerializer
)
