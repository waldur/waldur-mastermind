...
from waldur_openstack.openstack_tenant import apps as ot_apps
...
from waldur_openstack.openstack_tenant import serializers as ot_serializers

from .. import optimizers, register, serializers

OptimizedPreset = collections.namedtuple(
    'OptimizedPreset',
    ('preset', 'flavor', 'quantity', 'price', 'flavor_price', 'storage_price'),
)
    ...
    def _get_flavor_price(self, service, flavor):
        ...
        try:
            ...
            raise optimizers.OptimizationError(
                'Price is not defined for flavor "%s".' % flavor.name
            )
        return item.value * self.HOURS_IN_DAY

    def _get_cheapest_flavor(self, service, suitable_flavors):
        priced_flavors = [
            (flavor, self._get_flavor_price(service, flavor))
...
class OptimizedOpenStackTenantSerializer(serializers.OptimizedServiceSerializer):
    service = rf_serializers.HyperlinkedRelatedField(
        view_name='openstacktenant-detail', lookup_field='uuid', read_only=True,
    )
    optimized_presets = OptimizedPresetSerializer(many=True)


register.Register.register_serializer(
    ot_apps.OpenStackTenantConfig.service_name, OptimizedOpenStackTenantSerializer
)
