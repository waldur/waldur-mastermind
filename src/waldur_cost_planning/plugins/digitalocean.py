...
from waldur_digitalocean import cost_tracking as do_cost_tracking
...
OptimizedPreset = collections.namedtuple(
    'OptimizedPreset', ('preset', 'size', 'quantity', 'price')
)
...
class DigitalOceanOptimizer(optimizers.Optimizer):
    ...
    def _get_size_prices(self, sizes, service):
        ...
        service_price_list_items = utils.get_service_price_list_items(
            service, do_models.Droplet
        )
        size_prices = {
            item.key: item.value
            for item in service_price_list_items
            if item.item_type == do_cost_tracking.DropletStrategy.Types.FLAVOR
        }
        return {
            size: size_prices.get(size.name, size.price) * self.HOURS_IN_DAY
            for size in sizes
        }
    ...
    def optimize(self, deployment_plan, service):
        ...
        for item in deployment_plan.items.all():
            ...
            sizes = [
                size
                for size in sizes
                if size.cores >= preset.cores
                and size.ram >= preset.ram
                and size.disk >= preset.storage
            ]
            if not sizes:
                ...
                raise optimizers.OptimizationError(
                    'It is impossible to create a droplet for preset %s. It is too big.'
                    % preset_as_str
                )
            optimal_size = min(sizes, key=lambda size: size_prices[size])
            optimized_presets.append(
                OptimizedPreset(
                    preset=preset,
                    size=optimal_size,
            ...
            price += size_prices[optimal_size] * item.quantity
        return OptimizedDigitalOcean(
            price=price, service=service, optimized_presets=optimized_presets
        )


register.Register.register_optimizer(
    do_apps.DigitalOceanConfig.service_name, DigitalOceanOptimizer
)
...
class OptimizedDigitalOceanSerializer(serializers.OptimizedServiceSerializer):
    service = rf_serializers.HyperlinkedRelatedField(
        view_name='digitalocean-detail', lookup_field='uuid', read_only=True,
    )
    optimized_presets = OptimizedPresetSerializer(many=True)


register.Register.register_serializer(
    do_apps.DigitalOceanConfig.service_name, OptimizedDigitalOceanSerializer
)
