...
from .models import Host, Item, ITService, SlaHistory
...
def pull_sla(host_uuid):
    ...
    try:
        ...
        logger.warning(
            'Unable to pull SLA for host with UUID %s, because it is gone', host_uuid
        )
        return
...
def update_sla(sla_type):
    ...
    if sla_type == 'yearly':
        period = dt.year
        start_time = int(
            datetime.datetime.strptime('01/01/%s' % dt.year, '%d/%m/%Y').strftime("%s")
        )
...
def update_itservice_sla(itservice_pk, period, start_time, end_time):
    logger.debug(
        'Updating SLAs for IT Service with PK %s. Period: %s, start_time: %s, end_time: %s',
        itservice_pk,
        period,
        start_time,
        end_time,
    ...
    try:
        ...
        logger.warning(
            'Unable to update SLA for IT Service with PK %s, because it is gone',
            itservice_pk,
        )
        return
    ...
    try:
        ...
        if itservice.host and itservice.host.scope and itservice.is_main:
            ResourceSla.objects.update_or_create(
                ...
                period=period,
                defaults={'value': current_sla, 'agreed_value': itservice.agreed_sla},
            )

        if itservice.backend_trigger_id:
            # update connected events
            events = backend.get_trigger_events(
                itservice.backend_trigger_id, start_time, end_time
            )
            for event in events:
                event_state = 'U' if int(event['value']) == 0 else 'D'
                entry.events.get_or_create(
                    timestamp=int(event['timestamp']), state=event_state
                )
        ...
        logger.warning(
            'Unable to update SLA for IT Service %s (ID: %s). Reason: %s',
            itservice.name,
            itservice.backend_id,
            e,
        )
    logger.debug(
        'Successfully updated SLA for IT Service %s (ID: %s)',
        itservice.name,
        itservice.backend_id,
    )
...
def update_monitoring_items():
    ...
    for host in hosts:
        for config in host.MONITORING_ITEMS_CONFIGS:
            update_host_scope_monitoring_items.delay(
                host.uuid.hex,
...
@shared_task
def update_host_scope_monitoring_items(
    host_uuid, zabbix_item_key, monitoring_item_name
):
    host = Host.objects.get(uuid=host_uuid)
    ...
    if Item.objects.filter(template__hosts=host, key=zabbix_item_key).exists():
        value = host.get_backend().get_item_last_value(
            host.backend_id, key=zabbix_item_key
        )
        ...
        logger.debug(
            'Successfully updated monitoring item %s for host %s (%s). Current value: %s.',
        ...
        logger.debug(
            'Host %s (UUID: %s) does not have monitoring item %s.',
            host.visible_name,
            host.uuid.hex,
            monitoring_item_name,
        )
...
def after_creation_monitoring_item_update(host_uuid, config):
    ...
    return (
        item_value in config.get('after_creation_update_terminate_values', [])
...
class SMSTask(core_tasks.Task):
    ...
    def execute(self, settings, message, phone):
        ...
        if sender and recipient and '{phone}' in recipient:
            send_mail(
                '', message, sender, [recipient.format(phone=phone)], fail_silently=True
            )
        else:
            logger.warning(
                'SMS was not sent, because `sms_email_from` and `sms_email_rcpt` '
...
def pull_hosts():
    pullable_hosts = Host.objects.exclude(
        backend_id=''
    )  # Cannot pull hosts without backend_id
