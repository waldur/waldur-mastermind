...
urlpatterns = [
    ...
    re_path(
        r"^marketplace-checklists-categories/(?P<category_uuid>[a-f0-9]+)/checklists/$",
        views.CategoryChecklistsView.as_view({"get": "list"}),
    ),
    re_path(
        r"^marketplace-checklists/(?P<checklist_uuid>[a-f0-9]+)/stats/$",
        views.StatsView.as_view(),
    ),
    re_path(
        r"^projects/(?P<project_uuid>[a-f0-9]+)/marketplace-checklists/$",
        views.ProjectStatsView.as_view(),
    ),
    re_path(
        r"^customers/(?P<customer_uuid>[a-f0-9]+)/marketplace-checklists/(?P<checklist_uuid>[a-f0-9]+)/$",
        views.CustomerStatsView.as_view(),
        name="marketplace-checklists-customer-stats",
    ),
    re_path(
        r"^customers/(?P<customer_uuid>[a-f0-9]+)/marketplace-checklists/$",
        views.CustomerChecklistUpdateView.as_view(),
        name="marketplace-checklists-customer",
    ),
    re_path(
        r"^marketplace-checklists/(?P<checklist_uuid>[a-f0-9]+)/answers/$",
        views.AnswersListView.as_view({"get": "list"}),
    ),
    re_path(
        r"^marketplace-checklists/(?P<checklist_uuid>[a-f0-9]+)/answers/submit/$",
        views.AnswersSubmitView.as_view({"post": "create"}),
    ),
    re_path(
        r"^marketplace-checklists/(?P<checklist_uuid>[a-f0-9]+)/user/(?P<user_uuid>[a-f0-9]+)/answers/$",
        views.UserAnswersListView.as_view({"get": "list"}),
    ),
    re_path(
        r"^users/(?P<user_uuid>[a-f0-9]+)/marketplace-checklist-stats/$",
