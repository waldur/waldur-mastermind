...
class CreditReportsVisibilityTest(test.APITestCase):
    """Permissions of the per-project credit reports display flag."""
    ...
    def test_hidden_by_default(self):
        self.assertFalse(self.fixture.project.display_credit_reports)

    def test_field_is_visible_to_project_users(self):
        url = factories.ProjectFactory.get_url(self.fixture.project)
        self.client.force_authenticate(self.fixture.owner)
        response = self.client.get(url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(response.data["display_credit_reports"])

    def test_staff_can_enable_reports_for_a_project(self):
        url = factories.ProjectFactory.get_url(self.fixture.project)
        self.client.force_authenticate(self.staff_user)
        response = self.client.patch(url, {"display_credit_reports": True})

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.fixture.project.refresh_from_db()
        self.assertTrue(self.fixture.project.display_credit_reports)

    def test_non_staff_cannot_enable_reports_for_a_project(self):
        url = factories.ProjectFactory.get_url(self.fixture.project)
        self.client.force_authenticate(self.fixture.owner)
        response = self.client.patch(url, {"display_credit_reports": True})
