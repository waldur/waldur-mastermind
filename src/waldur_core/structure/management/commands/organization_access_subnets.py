...
def get_merged_subnets():
    subnets_queryset = models.AccessSubnet.objects.all().values_list("inet", flat=True)
    ...
    for version, version_networks in grouped_networks.items():
        merged_version_networks = list(ipaddress.collapse_addresses(version_networks))
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest="output",
            default=None,
            help="Specifies file to which the merged subnets will be written. The output will be printed to stdout by default.",
        )

    def handle(self, *args, **options):
        merged_subnets = get_merged_subnets()
