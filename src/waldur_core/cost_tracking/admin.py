...
def _get_content_type_queryset(models_list):
    ...
    content_type_ids = {
        c.id for c in ContentType.objects.get_for_models(*models_list).values()
    }
        ...
        if self.value():
            ...
            if model:
                return queryset.filter(
                    resource_content_type=ContentType.objects.get_for_model(model)
                )
        return queryset


class DefaultPriceListItemAdmin(
    core_admin.ExtraActionsMixin, structure_admin.ChangeReadonlyMixin, admin.ModelAdmin
):
    list_display = (
        'full_name',
        'item_type',
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "resource_content_type":
            kwargs["queryset"] = _get_content_type_queryset(
                CostTrackingRegister.registered_resources.keys()
            )
        return super(DefaultPriceListItemAdmin, self).formfield_for_foreignkey(
    ...
    def init_registered(self, request):
        ...
        if created_items:
            message = ungettext(
                ...
                _('Price items were created: %s.')
                % ', '.join(item.name for item in created_items),
                len(created_items),
            )
            ...
            self.message_user(
                request,
                _('Price items for all registered resources have been updated.'),
            )
    ...
    def delete_not_registered(self, request):
        ...
        if deleted_items_names:
            ...
            self.message_user(
                request, _('Nothing to delete. All default price items are registered.')
            )
    ...
    def recalulate_current_estimates(self, request):
        ...
        self.message_user(
            request,
            _(
                'Total and consumed value were successfully recalculated for all price estimates.'
            ),
        )
        return redirect(reverse('admin:cost_tracking_defaultpricelistitem_changelist'))
...
class PriceListItemAdmin(admin.ModelAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        return super(PriceListItemAdmin, self).formfield_for_foreignkey(
            db_field, request, **kwargs
        )
...
class ScopeTypeFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        estimated_models = [
            (core_utils.serialize_class(model), name)
