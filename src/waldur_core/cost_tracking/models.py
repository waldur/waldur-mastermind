...
from waldur_core.cost_tracking import ConsumableItem, managers
...
class PriceEstimate(LoggableMixin, core_models.UuidMixin, core_models.DescendantMixin):
    r""" Store prices based on both estimates and actual consumption.
    ...
    details = JSONField(
        default=dict,
        help_text=_('Saved scope details. Field is populated on scope deletion.'),
    )
    parents = models.ManyToManyField(
        'PriceEstimate', related_name='children', help_text=_('Price estimate parents')
    )
    ...
    consumed = models.FloatField(
        default=0, help_text=_('Price for resource until now.')
    )
    ...
    def is_resource_estimate(self):
        return issubclass(
            self.content_type.model_class(), structure_models.ResourceMixin
        )

    def get_previous(self):
        """ Get estimate for the same scope for previous month. """
        month, year = (
            (self.month - 1, self.year) if self.month != 1 else (12, self.year - 1)
        )
    ...
    def create_ancestors(self):
        ...
        for scope_parent in scope_parents:
            parent, created = PriceEstimate.objects.get_or_create(
                scope=scope_parent, month=self.month, year=self.year
            )
    ...
    def init_details(self):
        ...
        if self.is_resource_estimate():
            ...
            ] = self.scope.service_project_link.service.settings.name
    ...
    def create_historical(cls, resource, configuration, date):
        ...
        """
        price_estimate = cls.objects.create(
            scope=resource, month=date.month, year=date.year
        )
    ...
    def _get_price(self, consumed):
        ...
        consumables_prices = {
            (item.item_type, item.key): item.minute_rate for item in price_list_items
        }
        total = 0
        for consumable_item, usage in consumed.items():
            try:
                total += (
                    consumables_prices[(consumable_item.item_type, consumable_item.key)]
                    * usage
                )
            except KeyError:
                logger.debug(
                    'Price list item for consumable "%s" does not exist.'
                    % consumable_item
                )
        return total
    ...
    def _check_is_updatable(self):
        ...
        if not ConsumptionDetails.objects.filter(price_estimate=self).exists():
            raise EstimateUpdateError(
                'Cannot update consumed for price estimate that does not have consumption details.'
            )
        if not self.is_resource_estimate() or not self.scope:
            raise EstimateUpdateError(
                'Cannot update consumed for price estimate that is not related to resource.'
            )
    ...
    def get_scope_name(self):
        if self.scope:
            if isinstance(
                self.scope,
                (structure_models.ServiceProjectLink, structure_models.Service),
            ):
    ...
    def update_resource_estimate(resource, new_configuration, raise_exception=False):
        ...
        price_estimate, created = PriceEstimate.objects.get_or_create_current(
            scope=resource
        )
        if created:
            price_estimate.create_ancestors()
        consumption_details, _ = ConsumptionDetails.objects.get_or_create(
            price_estimate=price_estimate
        )
...
class ConsumableItemsField(JSONField):
    ...
    def get_prep_value(self, value):
        if not isinstance(value, dict):
            raise TypeError(
                'ConsumableItemsField value should be dict. Received: %s' % value
            )
        if any([not isinstance(item, ConsumableItem) for item in value]):
            raise TypeError(
                'ConsumableItemsField keys should be instances of ConsumableItem class.'
            )
    ...
    def _serialize(self, value):
        return [
            {'usage': usage, 'item_type': item.item_type, 'key': item.key}
...
class ConsumptionDetails(core_models.UuidMixin, TimeStampedModel):
    ...
    price_estimate = models.OneToOneField(
        PriceEstimate, related_name='consumption_details', on_delete=models.CASCADE
    )
    configuration = ConsumableItemsField(
        default=dict, help_text=_('Current resource configuration.')
    )
    last_update_time = models.DateTimeField(
        help_text=_('Last configuration change time.')
    )
    consumed_before_update = ConsumableItemsField(
        default=dict,
        help_text=_('How many consumables were used by resource before last update.'),
    )
    ...
    def update_configuration(self, new_configuration):
        ...
        if now.month != self.price_estimate.month:
            raise ConsumptionDetailUpdateError(
                'It is possible to update consumption details only for current month.'
            )
        ...
        for consumable_item, usage in self.configuration.items():
            ...
            self.consumed_before_update[consumable_item] = (
                self.consumed_before_update.get(consumable_item, 0)
                + consumed_after_modification
            )
    ...
    def consumed_in_month(self):
        ...
        month_end = core_utils.month_end(
            datetime.date(self.price_estimate.year, self.price_estimate.month, 1)
        )
    ...
    def _get_consumed(self, time):
        ...
        if minutes_from_last_update < 0:
            raise ConsumptionDetailCalculateError(
                'Cannot calculate consumption if time < last modification date.'
            )
        _consumed = {}
        for consumable_item in set(
            list(self.configuration.keys()) + list(self.consumed_before_update.keys())
        ):
            after_update = (
                self.configuration.get(consumable_item, 0) * minutes_from_last_update
            )
...
class AbstractPriceListItem(models.Model):
    ...
    value = models.DecimalField(
        "Hourly rate", default=0, max_digits=13, decimal_places=7
    )
...
class DefaultPriceListItem(
    core_models.UuidMixin, core_models.NameMixin, AbstractPriceListItem
):
    """
    ...
    item_type = models.CharField(
        max_length=255,
        help_text=_('Type of price list item. Examples: storage, flavor.'),
    )
    key = models.CharField(
        max_length=255,
        help_text=_(
            'Key that corresponds particular consumable. Example: name of flavor.'
        ),
    )
    resource_content_type = models.ForeignKey(
        on_delete=models.CASCADE, to=ContentType, default=None
    )
    tracker = FieldTracker()

    def __str__(self):
        return 'Price list item %s: %s = %s for %s' % (
            self.name,
            self.key,
            self.value,
    ...
    def get_consumable_items_pretty_names(cls, resource_content_type, consumable_items):
        ...
        price_list_items = cls.objects.filter(
            query, resource_content_type=resource_content_type
        )
        return {
            ConsumableItem(item_type, key): name
            for item_type, key, name in price_list_items.values_list(
                'item_type', 'key', 'name'
            )
        }
    ...
    def init_from_registered_resources(cls):
        ...
        with transaction.atomic():
            for resource_class in CostTrackingRegister.registered_resources:
                ...
                for consumable_item in CostTrackingRegister.get_consumable_items(
                    resource_class
                ):
                    (
                        price_list_item,
                        created,
                    ) = cls._create_or_update_default_price_list_item(
    ...
    @classmethod
    def _create_or_update_default_price_list_item(
        cls, resource_content_type, consumable_item
    ):
...
class PriceListItem(core_models.UuidMixin, AbstractPriceListItem):
    ...
    default_price_list_item = models.ForeignKey(
        on_delete=models.CASCADE, to=DefaultPriceListItem
    )
    ...
    def clean(self):
        ...
        valid_resources = SupportedServices.get_related_models(self.service)[
            'resources'
        ]
    ...
    def get_for_resource(resource):
        ...
        default_items = set(
            DefaultPriceListItem.objects.filter(
                resource_content_type=resource_content_type
            )
        )
        ...
        items = set(
            PriceListItem.objects.filter(
                default_price_list_item__in=default_items, service=service
            ).select_related('default_price_list_item')
        )
