...
class PriceListItemListTest(BaseCostTrackingTest):
    ...
    def test_user_with_access_to_service_can_see_services_price_list(self, user):
        ...
        self.assertIn(
            self.price_list_item.uuid.hex, [el['uuid'] for el in response.data]
        )
    ...
    def test_price_list_can_be_filtered_by_service(self):
        ...
        self.assertNotIn(
            other_price_list_item.uuid.hex, [el['uuid'] for el in response.data]
        )
...
class PriceListItemCreateTest(BaseCostTrackingTest):
    def setUp(self):
        ...
        self.valid_data = {
            'service': structure_factories.TestServiceFactory.get_url(self.service),
            'default_price_list_item': factories.DefaultPriceListItemFactory.get_url(
                self.default_price_list_item
            ),
            'value': 100,
            'units': 'UAH',
        }
    ...
    def test_user_with_permissions_can_create_price_list_item(self, user):
        ...
        response = self.client.post(
            factories.PriceListItemFactory.get_list_url(), data=self.valid_data
        )
        ...
        self.assertTrue(
            models.PriceListItem.objects.filter(
                ...
                default_price_list_item=self.default_price_list_item,
            ).exists()
        )
...
class PriceListItemUpdateTest(BaseCostTrackingTest):
    ...
    def test_user_with_permissions_can_update_price_list_item(self, user):
        ...
        response = self.client.patch(
            factories.PriceListItemFactory.get_url(self.price_list_item), data=data
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        reread_price_list_item = models.PriceListItem.objects.get(
            id=self.price_list_item.id
        )
