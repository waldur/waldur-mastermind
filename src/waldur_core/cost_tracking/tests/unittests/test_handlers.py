...
class ResourceUpdateTest(TransactionTestCase):
    ...
    @freeze_time(
        '2016-08-08 11:00:00', tick=True
    )  # freeze time to avoid bugs in the end of a month.
    def test_consumption_details_of_resource_is_keeped_up_to_date(self):
        ...
        price_estimate = models.PriceEstimate.objects.get(
            scope=resource, month=today.month, year=today.year
        )
    ...
    def test_price_estimate_of_resource_is_keeped_up_to_date(self):
        ...
        with freeze_time(start_time):
            ...
            expected = (
                int((month_end - start_time).total_seconds() / 60)
                * self.price_list_item.minute_rate
                * old_disk
            )
        ...
        with freeze_time(change_time):
            ...
            expected = (
                int((change_time - start_time).total_seconds() / 60)
                * self.price_list_item.minute_rate
                * old_disk
                + int((month_end - change_time).total_seconds() / 60)
                * self.price_list_item.minute_rate
                * new_disk
            )
            self.assertAlmostEqual(price_estimate.total, expected)
    ...
    def test_price_estimate_of_resource_ancestors_is_keeped_up_to_date(self):
        ...
        with freeze_time(start_time):
            ...
            ancestors = [
                ...
                resource.service_project_link.service,
                resource.service_project_link.project,
                resource.service_project_link.project.customer,
            ]
            ...
            for ancestor in ancestors:
                ancestor_estimate = models.PriceEstimate.objects.get(
                    scope=ancestor, month=today.month, year=today.year
                )
    ...
    def test_historical_estimates_are_initialized(self):
        ...
        for scope in [resource] + ancestors:
            self.assertTrue(
                models.PriceEstimate.objects.filter(scope=scope, month=7, year=2016)
            )
            self.assertTrue(
                models.PriceEstimate.objects.filter(scope=scope, month=8, year=2016)
            )
...
class ScopeDeleteTest(TransactionTestCase):
    ...
    def test_all_estimates_are_deleted_on_customer_deletion(self):
        ...
        for scope in (
            ...
            self.assertFalse(
                models.PriceEstimate.objects.filter(scope=scope).exists(),
                'Unexpected price estimate exists for %s %s'
                % (scope.__class__.__name__, scope),
            )
    ...
    def test_estimate_is_recalculated_on_resource_deletion(self):
        ...
        expected = (
            2 * self.price_list_item.value * self.resource.disk
        )  # resource has been working for 2 hours
        for scope in (self.spl, self.service, self.project, self.customer):
            self.assertEqual(
                models.PriceEstimate.objects.get_current(scope).total, expected
            )
