...
from waldur_core.cost_tracking import ConsumableItem, models
...
class ConsumptionDetailsTest(TransactionTestCase):
    ...
    def test_cannot_update_configuration_for_previous_month(self):
        with freeze_time("2016-09-01"):
            self.assertRaises(
                ...
                lambda: self.consumption_details.update_configuration(
                    {self.storage_item: 2048}
                ),
            )
    ...
    def test_consumed_in_month(self):
        ...
        expected = (
            ...
            * old_configuration[self.storage_item]
            + int((month_end - change_time).total_seconds() / 60)
            * new_configuration[self.storage_item]
        )
        self.assertEqual(
            self.consumption_details.consumed_in_month[self.storage_item], expected
        )
    ...
    def test_consumed_until_now(self):
        ...
        with freeze_time(now_time):
            expected = (
                2 * 60 * old_configuration[self.storage_item]
                + 1  # Resource worked two hours with old configuration
                * 60
                * new_configuration[self.storage_item]
            )  # And one hour with new configuration
            self.assertEqual(
                self.consumption_details.consumed_until_now[self.storage_item], expected
            )
...
class PriceListItemTest(TransactionTestCase):
    def test_get_for_resource(self):
        ...
        item = models.PriceListItem.objects.create(
            default_price_list_item=default_item2, service=service
        )
