...
from waldur_core.cost_tracking import CostTrackingRegister, models, tasks
...
class RecalculateEstimateTest(TransactionTestCase):
    ...
    def test_consumed_is_recalculated_properly_for_resource(self):
        ...
        expected = (
            working_minutes * self.price_list_item.minute_rate * self.resource.disk
        )
    ...
    def test_consumed_is_recalculated_properly_for_ancestors(self):
        ...
        with freeze_time(calculation_time):
            ...
            price_estimates = [
                ...
                for ancestor in (self.customer, self.service, self.spl, self.project)
            ]
        ...
        for price_estimate in price_estimates:
            message = (
                'Price estimate "consumed" is calculated wrongly for "%s". Real value: %s, expected: %s.'
    ...
    def test_new_estimates_are_created_in_new_month(self):
        ...
        for price_estimate in price_estimates:
            message = (
                'Price estimate "total" is calculated wrongly for "%s". Real value: %s, expected: %s.'
        ...
        for price_estimate in price_estimates:
            ...
            self.assertAlmostEqual(
                price_estimate.consumed, expected_consumed, msg=message
            )
