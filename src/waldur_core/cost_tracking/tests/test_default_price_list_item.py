...
from .. import models
from . import base_test, factories
...
class DefaultPriceListItemListTest(base_test.BaseCostTrackingTest):
    def setUp(self):
        ...
        self.users['regular_user'] = structure_factories.UserFactory(
    ...
    def test_user_with_access_to_service_can_see_services_price_list(self, user):
        ...
        response = self.client.get(
            factories.DefaultPriceListItemFactory.get_list_url(), data
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn(
            self.default_price_list_item.uuid.hex, [el['uuid'] for el in response.data]
        )
