...
class PriceEstimateListTest(BaseCostTrackingTest):
    def setUp(self):
        ...
        self.project_price_estimate = factories.PriceEstimateFactory(
            scope=self.project, year=2015, month=7
        )
    ...
    def test_user_can_see_price_estimate_for_his_project(self, user):
        ...
        self.assertIn(
            self.project_price_estimate.uuid.hex, [obj['uuid'] for obj in response.data]
        )
    ...
    def test_user_cannot_see_price_estimate_for_not_his_project(self, user):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertNotIn(
            other_price_estimate.uuid.hex, [obj['uuid'] for obj in response.data]
        )
    ...
    def test_user_can_filter_price_estimates_by_date(self):
        ...
        response = self.client.get(
            ...
            data={
                'date': '{}.{}'.format(
                    self.link_price_estimate.year, self.link_price_estimate.month
                )
            },
        )
    ...
    def test_user_can_filter_price_estimates_by_date_range(self):
        ...
        response = self.client.get(
            ...
            data={
                'start': '{}.{}'.format(
                    self.link_price_estimate.year, self.link_price_estimate.month + 1
                ),
                'end': '{}.{}'.format(
                    self.project_price_estimate.year,
                    self.project_price_estimate.month + 1,
                ),
            },
        )
    ...
    def test_user_receive_error_on_filtering_by_not_visible_for_him_object(self):
        ...
        self.client.force_authenticate(self.users['administrator'])
        response = self.client.get(
            factories.PriceEstimateFactory.get_list_url(), data=data
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_user_can_define_children_visibility_depth(self):
        customer_price_estimate = factories.PriceEstimateFactory(
            scope=self.customer, year=2015, month=7
        )
        customer_price_estimate.children.add(self.project_price_estimate)
        spl_price_estimate = factories.PriceEstimateFactory(
            scope=self.service_project_link, year=2015, month=7
        )
        ...
        response = self.client.get(
            factories.PriceEstimateFactory.get_url(customer_price_estimate),
            data={'depth': 1},
        )
        ...
        self.assertEqual(
            project_estimate_data['uuid'], self.project_price_estimate.uuid.hex
        )
...
class PriceEstimateUpdateTest(BaseCostTrackingTest):
    ...
    def test_price_estimate_scope_cannot_be_updated(self):
        ...
        self.valid_data[
            'scope'
        ] = structure_factories.TestServiceProjectLinkFactory.get_url(
            other_service_project_link
        )

        self.client.force_authenticate(self.users['staff'])
        response = self.client.patch(
            factories.PriceEstimateFactory.get_url(self.price_estimate),
...
class PriceEstimateDeleteTest(BaseCostTrackingTest):
    ...
    def test_autocreated_price_estimate_cannot_be_deleted(self):
        ...
        response = self.client.delete(
            factories.PriceEstimateFactory.get_url(self.project_price_estimate)
        )
...
class ScopeTypeFilterTest(BaseCostTrackingTest):
    def setUp(self):
        ...
        self.estimates = {
            ...
            'service_project_link': models.PriceEstimate.objects.get(
                scope=self.service_project_link
            ),
...
class CustomerFilterTest(BaseCostTrackingTest):
    def setUp(self):
        ...
        self.estimates = {
            models.PriceEstimate.objects.get(scope=scope) for scope in scopes
        }
    ...
    def test_user_can_filter_price_estimate_by_customer_uuid(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            {estimate['uuid'] for estimate in response.data},
