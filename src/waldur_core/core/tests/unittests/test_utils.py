...
class TestFormatTimeAndValueToSegmentList(unittest.TestCase):
    ...
    def test_function_sums_values_in_segments_right(self):
        ...
        time_and_value_list = (
            first_segment_time_value_list + second_segment_time_value_list
        )
        # when
        segment_list = utils.format_time_and_value_to_segment_list(
        ...
        expected_first_segment_value = sum(
            [value for _, value in first_segment_time_value_list]
        )
        expected_second_segment_value = sum(
            [value for _, value in second_segment_time_value_list]
        )
