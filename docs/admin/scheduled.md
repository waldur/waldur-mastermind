...
| `scim-hourly-entitlement-reconciliation` | `waldur_core.users.scim.sync_recent_entitlements` | 1 hour | Syncs SCIM entitlements for users with recent role changes.<br><br> Finds users with active project roles modified in the last 2 hours and synchronizes their entitlements with the external SCIM service. Processes users in batches of 20. |
