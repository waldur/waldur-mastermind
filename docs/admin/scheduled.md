...
| `send-monthly-invoicing-reports-about-customers` | `invoices.send_monthly_invoicing_reports_about_customers` | Triggered programmatically | Send monthly invoicing reports via email to configured recipients. Triggered by `create_monthly_invoices` (when grace period is 0) or `finalize_previous_invoices` (when grace period > 0) after all invoices are finalized. No longer runs on a cron schedule. |
...
| `waldur-create-invoices` | `invoices.create_monthly_invoices` | Monthly (1st day of month at midnight) | - For every customer, finalize invoices for previous months (PENDING → CREATED when grace period is 0, or PENDING → PENDING_FINALIZATION when grace period > 0).<br> - Create new invoice for every customer in current month if not created yet.<br> - When grace period is 0, also triggers reports and notifications. |
| `waldur-finalize-invoices` | `invoices.finalize_previous_invoices` | Cron: `0 * 1-3 * * (m/h/dM/MY/d)` | Runs hourly on the 1st–3rd of each month. Finalizes invoices in PENDING_FINALIZATION state after the configured grace period has elapsed. No-op when no PENDING_FINALIZATION invoices exist or when the grace period has not yet elapsed. Triggers reports and notifications after all invoices are finalized. |
